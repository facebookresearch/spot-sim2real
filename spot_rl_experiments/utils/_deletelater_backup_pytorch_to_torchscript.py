...
class PolicyConverter:
    ...
    def convert_from_pytorch_to_torchscript(self, save_path, observations):
        ...
        torch.save(
            {
                "std": self.policy.action_distribution.std,
                "num_recurrent_layers": self.policy.net.num_recurrent_layers,
                "RL_PPO_HIDDEN_SIZE": self.config.habitat_baselines.rl.ppo.hidden_size,
                "MIN_LOG_STD": self.config.habitat_baselines.rl.policy.main_agent.action_dist.min_log_std,
                "MAX_LOG_STD": self.config.habitat_baselines.rl.policy.main_agent.action_dist.max_log_std,
            },
            save_path["std"],
        )
        ...
        print("Loaded std dict", torch.load(save_path["std"]))
        self.std = torch.load(save_path["std"])["std"].to(self.device)

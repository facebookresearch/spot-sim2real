...
def parse_yaml(file_path):
    ...
    for episode, details in episodes.items():
        ...
        save_logs_as_json()
        action_taker.spot.set_arm_joint_positions(
            positions=action_taker.get_env().stow_arm_joint_angles, travel_time=1.00
        )
        input("Waiting to open gripper")
        action_taker.spot.open_gripper()
        print('Finished resetting arm')

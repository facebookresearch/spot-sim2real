...
if __name__ == "__main__":
    ...
    while contnue:
        if in_fre_lab:
            ...
            hand_T_intel = mn.Matrix4(np.identity(4).T.tolist()) if place_point_generation_src == 0 else hand_T_intel
            image_resps = [
            ...
            if place_point_generation_src == 1:
                ...
                image_in_gripper = plot_intel_point_in_gripper_image(gripper_resps, hand_T_intel, placexyzbeforeconv)
            ...
            print("Place point before offset adjustment", placexyz, "Place point before any transformation", placexyzbeforeconv)
            # x, y, _ = spot.xy_yaw_global_to_home(*placexyz[:2], 0)
            # placexyz[:2] = np.array([x, y])
            placexyz[-1] += 0.1  # static offset in height
            print("Place point after offset adjustment", placexyz, "Place point before any transformation", placexyzbeforeconv)
            print("Ideal 1.1380085945129395, 0.006437085103243589, 0.8053807616233826 in HOME frame")

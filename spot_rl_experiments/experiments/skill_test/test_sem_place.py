...
if __name__ == "__main__":
    ...
    while contnue:
        if in_fre_lab:
            ...
            hand_T_intel = mn.Matrix4(np.load("gripper_T_intel.npy").T.tolist())
            image_resps = [
                image_response_to_cv2(image_resp) for image_resp in image_resps
            ]
            vision_T_hand = spot.get_magnum_Matrix4_spot_a_T_b(
                "vision", "hand_color_image_sensor", snapshot_tree
            )
            vision_T_hand = vision_T_hand.__matmul__(hand_T_intel)  # vision_T_intel
            (
                owlvitmodel,
                processor,
                sammodel,
            ...
            # placexyz[-1] += 0.3 #static offset in height
            print(placexyz, placexyzbeforeconv)
            print(
                "Ideal 1.0551527738571167, -0.00035699590807780623, 0.8063808679580688"
            )

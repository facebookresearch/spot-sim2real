...
if __name__ == "__main__":
    ...
    while contnue:
        if in_fre_lab:
            ...
            gripper_images = [
                image_response_to_cv2(gripper_resp) for gripper_resp in gripper_resps
            ]
            ...
            hand_T_intel = (
                np.load("gripper_T_intel.npy")
                if place_point_generation_src
                else np.identity(4)
            )
            ...
            if place_point_generation_src == 1:
                ...
                cv2.imwrite(
                    "table_detection.png",
                    np.hstack([intel_img_with_vis, image_in_gripper]),
                )
            ...
            placexyz = convert_point_in_body_to_place_waypoint(
                mn.Vector3(*placexyz), spot
            )

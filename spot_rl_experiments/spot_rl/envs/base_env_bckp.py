...
from spot_rl.utils.utils_copy import FixSizeOrderedDictCombined, find_time_stamp_difference
...
class SpotBaseEnv(SpotRobotSubscriberMixin, gym.Env):
    ...
    def __init__(
        ...
        # }
        self.detections_buffer = FixSizeOrderedDictCombined(maxlen=DETECTIONS_BUFFER_LEN, topic_names=["detections", "filtered_depth", "viz"], anchor_topic_name="detections")
    ...
    def get_gripper_images(self, save_image=False):
        ...
        if self.parallel_inference_mode:
            ...
            for i in reversed(self.detections_buffer["detections"].copy()):
                find_time_stamp_difference(self.detections_buffer["detections"], self.detections_buffer["filtered_depth"])
                if (

...
class SpotNavEnv(SpotBaseEnv):
    ...
    def get_success(self, observations):
        ...
        vector_robot_to_target = vector_robot_to_target / np.linalg.norm(vector_robot_to_target)
        vector_forward_robot = np.array(self.curr_transform.transform_vector(mn.Vector3(1,0,0)))[[0,1]]
        vector_forward_robot = vector_forward_robot / np.linalg.norm(vector_forward_robot)
        angle_facing_target = np.dot(vector_robot_to_target,vector_forward_robot)
        print(f"Nav info: gps: {gps}; goal_heading of obs: {goal_heading}; facing_goal: {angle_facing_target}")
    ...
    def get_observations(self):
        ...
        x1 = vector_robot_to_target[1] * vector_forward_robot[0] - vector_robot_to_target[0] * vector_forward_robot[1]
        x2 = vector_robot_to_target[0] * vector_forward_robot[0] + vector_robot_to_target[1] * vector_forward_robot[1]
        rotation_delta =  np.arctan2(x1,x2)
        #breakpoint()
        self.goal_heading = wrap_heading(self.yaw + rotation_delta)
        print(f"goal_heading: {self.goal_heading}; self.yaw {self.yaw}; rotation_delta: {rotation_delta}")

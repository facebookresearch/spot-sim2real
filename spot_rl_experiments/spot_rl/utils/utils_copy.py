...
class FixSizeOrderedDict(OrderedDict):
    def __init__(self, *args, maxlen=0, name="", **kwargs):
        self._maxlen = maxlen
        self.name = name
...
def parse_timestamp(ros_timestamp_str):
    """
    Parses a timestamp string, attempting to include milliseconds if available.
    If parsing with milliseconds fails, it falls back to second precision.
    """
    ros_timestamp_in_seconds = float(ros_timestamp_str[:-9] + '.' + ros_timestamp_str[-9:])
    return datetime.fromtimestamp(ros_timestamp_in_seconds)
    

class FixSizeOrderedDictCombined(OrderedDict):
    def __init__(self, *args, maxlen=0, topic_names=[], anchor_topic_name="", **kwargs):
        self.topic_names = topic_names
        self.anchor_topic_name = anchor_topic_name
        assert self.anchor_topic_name in self.topic_names, f"Expected {self.anchor_topic_name} in list of topics {self.topic_names}"
        self.fixed_sized_dicts = {}
        self.last_timestamp_for_anchor_topic = None
        super().__init__(*args, **kwargs)

        for topic_name in self.topic_names:
            OrderedDict.__setitem__(self, topic_name, FixSizeOrderedDict(*args, maxlen=maxlen, name=f"{topic_name}", **kwargs))
            if topic_name != self.anchor_topic_name:
                OrderedDict.__setitem__(self, f"{topic_name}_staging_area", FixSizeOrderedDict(*args, maxlen=maxlen*2, name=f"{topic_name}_staging", **kwargs))
    def __setitem__(self, keys, value):
        
        assert isinstance(keys, tuple) and len(keys) == 2 , f"keys should be tuple but found {type(keys)} and len should be 2 but found {len(keys)}"
        topic_name, key = keys
        assert topic_name in self.topic_names, f"{topic_name} not initialized"
        if topic_name == self.anchor_topic_name:
            self[topic_name][key] = value
            self.last_timestamp_for_anchor_topic = key
            for topic_name_ in self.topic_names:
                if f"{topic_name_}_staging_area" in self and key in self[f"{topic_name_}_staging_area"]:
                    #print(f"staging area hit {topic_name_}")
                    self[topic_name_][key] = self[f"{topic_name_}_staging_area"][key]

        elif not self.last_timestamp_for_anchor_topic:
            self[f"{topic_name}_staging_area"][key] = value
        else:
            #convert timestamp to header
            current_time_stamp = parse_timestamp(key)
            last_time_stamp_of_anchor_topic = parse_timestamp(self.last_timestamp_for_anchor_topic)
            if abs(last_time_stamp_of_anchor_topic - current_time_stamp) <= timedelta(seconds=1):
                #print("Found close timed value")
                self[f"{topic_name}_staging_area"][key] = value

def find_time_stamp_difference(timestamp_lists1, timestamp_list2):
    for timestamp1 in timestamp_lists1:
        for timestamp2 in timestamp_list2:
            pass
            #print( abs( parse_timestamp(timestamp1) - parse_timestamp(timestamp2) ) <= timedelta(milliseconds=200))

if __name__ == "__main__":
    detections_buffer = FixSizeOrderedDictCombined(maxlen=30, topic_names=["detections", "filtered_depth", "viz"], anchor_topic_name="detections")
    detections_buffer["detections", str(5)] = "somedatapoint"
    #print(len(detections_buffer["detections"]))
    #print(detections_buffer["detections"]["5"])
    timestamp = "1731640696210438251"
    parse_timestamp(timestamp)
    print( abs(parse_timestamp(timestamp) - parse_timestamp("1731640697676001548")) <= timedelta(seconds=1.5) )
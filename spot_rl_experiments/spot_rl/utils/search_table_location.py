...
def convert_point_in_body_to_place_waypoint(
    point_in_body: mn.Vector3, spot: Spot
):  # 3d point
...
def plot_intel_point_in_gripper_image(
    ...
    # breakpoint()
    point3d_in_gripper: mn.Matrix3 = gripper_T_intel.transform_point(
        mn.Vector3(point3d_in_intel)
    )
    point3d_in_gripper: np.ndarray = np.array(
...
def detect_place_point_by_pcd_method(
    ...
    """
    depth_raw is intel_raw_depth
    percentile denotes threshold on width of the receptacle
    """
    h, w = img.shape[:2]
    ...
    pcd = generate_point_cloud(img, depth_raw, mask, [0, 0, 0, 0], fx, fy, cx, cy)
    ...
    point_in_body = np.array(
        body_T_hand.transform_point(mn.Vector3(*selected_point_in_gripper))
    )
    ...
    return point_in_body, selected_point_in_gripper, img_with_bbox

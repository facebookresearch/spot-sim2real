...
def convert_detections(detections, threshold, classes=[0]):
    # Get the bounding boxes, labels and scores from the detections dictionary.
    boxes = detections["boxes"].clone().cpu().numpy()
    labels = detections["labels"].clone().cpu().numpy()
    scores = detections["scores"].clone().cpu().numpy()
...
def convert_track_class_to_numpy_tracks(tracks):
    nunpy_tracks = []
    for track in tracks:
...
def annotate(tracks, frame, resized_frame, frame_width, frame_height, colors):
    for track in tracks:
        ...
        p1 = (
            int(x1 / resized_frame.shape[1] * frame_width),
            int(y1 / resized_frame.shape[0] * frame_height),
        )
        p2 = (
            int(x2 / resized_frame.shape[1] * frame_width),
            int(y2 / resized_frame.shape[0] * frame_height),
        )
        # Annotate boxes.

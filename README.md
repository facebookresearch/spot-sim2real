...
# --------
$ Home the robot at its dock
spot_reset_home
# --------

# Demo executution

# For the initial command without lang
python -m spot_rl.envs.skill_manager

# for any lang command
python -m spot_rl.envs.lang_env

# ASC (the 4 plushes)
spot_rl_mobile_manipulation_env 

# to stop the robot, go to estop tab and press s

# dock the robot
spot_rl_autodock 

# hijack control of robot
spot_keyboard_teleop
```
# directory: ~/asc_demo/spot-sim2real

## Script
1. `python -m spot_rl.envs.skill_manager && spot_rl_autodock`: The robot takes the plush from the sofa near the guitar to the basket.
2. `python -m spot_rl.envs.lang_env`: We ask the robot to "Move the water bottle from TV stand to sofa by the tv"
3. (deprecated for now) Take the bottle from the coffe table to the trash_can 
4. ``python -m spot_rl.envs.lang_env` We ask the robot to "Take the book from the sofa by the tv to the coffee table"
5. `spot_rl_mobile_manipulation_env ` The robot moves all plushes to the hamper:
    - two on the tv stand
    - one on the coffee table
    - one on each chair by the guitar
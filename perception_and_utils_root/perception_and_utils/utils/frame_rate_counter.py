...
class FrameRateCounter:
    ...
    def __init__(self):
        self.logger = logging.getLogger("FrameRateCounter")
    ...
    def stop(self):
        ...
        if elapsed_time_ns > 0:
            ...
            if self.window_index >= self.start_index:
                window_average = (self.window_index - self.start_index + 1) / (
            ...
            self.logger.info(
                f"Averate over last {self.window_size} records: {window_average: .2f} FPS"
            )

        return instantaneous_frame_rate, average_frame_rate

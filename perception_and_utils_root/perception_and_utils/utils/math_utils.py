...
def compute_avg_sophus_SE3_from_nplist(
    ...
    avg_a_T_b_quaternion = average_quaternion_numpy(a_T_b_quaternion_np, W=None)
    avg_sp_se3 = sp.SE3(
...
def average_quaternion_numpy(Q, W=None):
    """
    Compute the weighted average of a set of quaternions using eigenvalue decomposition.

    This function calculates the average of a set of quaternions by constructing a covariance-like
    matrix from the input quaternions and using eigenvalue decomposition to find the dominant
    quaternion. Optionally, the quaternions can be weighted using the provided weight array `W`.

    Args:
        Q : np.ndarray
            A 2D NumPy array of shape (n, 4), where each row represents a quaternion (x, y, z, w).
            Quaternions should be normalized.
        W : np.ndarray, optional
            A 1D NumPy array of shape (n,) representing the weights for each quaternion. If `W` is
            not provided, an unweighted average is computed. The weights should be non-negative.

    Returns:
        np.ndarray
            A 1D NumPy array of shape (4,) representing the average quaternion (x, y, z, w).
            The quaternion will be normalized.

    Notes:
        - Quaternions are assumed to be normalized, i.e., their magnitude should be 1.
        The function does not normalize the quaternions automatically.
        - If weights are provided, they influence how much each quaternion contributes to the average.
        - The returned quaternion is the eigenvector corresponding to the largest eigenvalue of
        the covariance matrix constructed from the weighted (or unweighted) quaternions.
    """
    if W is not None:
        Q *= W[:, None]

...
DU_SCALE = 4 # The point cloud resolution
...
SAVE_IMG_DIR = "/Users/jimmytyyang/Downloads/build_map/0215_run2" # The saving directory
        ...
        if episode_i == 0:
            ...
            for image_response, source in zip(image_responses, sources_for_map):
                if source is SpotCamIds.HAND_COLOR:
                    # Get the camera matrics and its parameter. Since the depth image is aligned to RGB, we get
                    # the parameter of RGB camera
                    source_rows_height = image_response.source.rows # 480
                    source_cols_width = image_response.source.cols # 640
                    fx = image_response.source.pinhole.intrinsics.focal_length.x # 552.0291012161067
                    fy = image_response.source.pinhole.intrinsics.focal_length.y # 552.0291012161067
                    cx = image_response.source.pinhole.intrinsics.principal_point.x # 320.0
                    cy = image_response.source.pinhole.intrinsics.principal_point.y # 240.0
                    break
            # Compute the hfov using camera parameters
            hfov = height_fy_to_hfov(source_rows_height, fy)

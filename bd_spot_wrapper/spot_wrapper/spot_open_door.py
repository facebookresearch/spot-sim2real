...
def get_images_as_cv2(robot, sources, format_flag):
    ...
    """
    if format_flag:
        pixel_format = ["PIXEL_FORMAT_RGB_U8"]*len(sources)
        image_client = robot.ensure_client(ImageClient.default_service_name)
        sources = [build_image_request(src, pixel_format=p) for src, p in zip(sources, pixel_format)]
        image_responses = image_client.get_image(sources)
    else:
        image_client = robot.ensure_client(ImageClient.default_service_name)
...
class RequestManager:
    ...
    def __init__(self, rgb_image_dict, image_dict, window_name):
        self.image_dict = image_dict
        self.rgb_image_dict = rgb_image_dict
        self.window_name = window_name
        self.handle_position_side_by_side = None
        self.hinge_position_side_by_side = None
        self._side_by_side = None
        self._rgb_side_by_side = None
    ...
    @property
    def rgb_side_by_side(self):
        """cv2.Image: Side by side rotated frontleft and frontright fisheye images"""
        if self._rgb_side_by_side is not None:
            return self._rgb_side_by_side
        ...
        self._rgb_side_by_side = np.hstack([fr_fisheye_image, fl_fisheye_image])
    ...
    @staticmethod
    def model_pred_location(image, text):
        texts = [text]
        ...
        handle_box = []
        handle_score = -float("inf")

        wall_box = []
        wall_score = -float("inf")

        for box, score, label in zip(boxes, scores, labels):
            box = [round(i, 2) for i in box.tolist()]
            if score >= score_threshold:
                print(f"Detected {text[label]} with confidence {round(score.item(), 3)} at location {box}")
                if score > handle_score and text[label] == "door handle":
                    handle_score = score
                    handle_box = box
                if score > wall_score and text[label] == "wall":
                    wall_score = score
                    wall_box = box

        if handle_score == -float("inf") or wall_score == -float("inf"):
            return None, None
        else:
            print("handle_box:", handle_box)
            print("wall_box:", wall_box)
            box_x = int((handle_box[0] + handle_box[2])/2)
            box_y = int((handle_box[1] + handle_box[3])/2)

            wall_x = int((wall_box[0] + wall_box[2])/2)
            wall_y = int((wall_box[1] + wall_box[3])/2)

            return [box_x, box_y], [wall_x, wall_y]

    def get_model_input_handle_and_hinge(self):
        """Open window showing the side by side fisheye images with on-screen prompts for user."""
        _draw_text_on_image(self.rgb_side_by_side, "Click handle.")
        cv2.imshow(self.window_name, self.rgb_side_by_side) # (640, 960) for self.side_by_side's shape
        cv2.waitKey(1)
        image = self.array_2_image(self.rgb_side_by_side)
        # [692.16, 211.43, 892.61, 341.03]
        self.handle_position_side_by_side, self.hinge_position_side_by_side = self.model_pred_location(image, ["door handle","wall"])
        print("handle_position_side_by_side:", self.handle_position_side_by_side)
        print("hinge_position_side_by_side:", self.hinge_position_side_by_side)
        if self.handle_position_side_by_side is None or self.hinge_position_side_by_side is None:
